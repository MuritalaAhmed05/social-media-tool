  ...
  const fetchAlbumData = async () => {
    ...
    try {
      const response = await fetch(
        `/api/downloader?platform=spotify&url=${encodeURIComponent(url.trim())}`
      );
          ...
          tracks: [
            {
              id: "1",
              title: item.title,
              artist: item.author?.name || "Spotify Artist",
              duration: "Preview",
              image: item.thumbnail,
              url: url
            }
        ...
        <div className="mt-6 w-full flex flex-col items-center">
          <div className="flex flex-col md:flex-row items-start space-y-4 md:space-y-0 md:space-x-9">
            ...
            <div>
              ...
              <p className="mt-2">
                Total Tracks: {albumData.data.total_tracks}
              </p>
